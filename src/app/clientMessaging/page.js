...
const ClientMessaging = () => {
    ...
    useEffect(() => {
        ...
        const fetchAndUpdateLocation = async () => {
            try {
                ...
                // Call API with the new location
        ...
        const intervalId = setInterval(fetchAndUpdateLocation, FETCH_INTERVAL);
    ...
    // Function to call API with the updated location
    const updateLocation = async (location) => {
        try {
            const authToken = JSON.parse(localStorage.getItem("user_token"));
            const response = await fetch('http://127.0.0.1:8000/api/update-location/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${authToken.access}` // Add authentication token if required
                },
                body: JSON.stringify({
    ...
    return (
        <div className='mt-2 mx-60 p-6 bg-white rounded-lg mb-2 shadow-lg'>
            <h2 className='font-bold'>Continuous location update</h2>
            {userLocation && (
