...
import React, { useState, useEffect } from 'react';
import { database } from '../firebase/firebase'; // Ensure the path is correct
import { ref, onValue, set, push, off } from "firebase/database"; // Import necessary functions from Firebase
...
const SmartDustbin = () => {
    ...
    useEffect(() => {
        ...
        const fetchDustbinData = () => {
            onValue(dustbinRef, (snapshot) => {
                ...
                if (data !== null) {
                    ...
                    if (percentage >= 90) { // Check if percentage is 90 or higher
                        sendNotificationRecord(data, percentage); // Call function to send data to Firebase
                    }
                }
            }, (error) => {
        ...
        return () => {
            off(dustbinRef); // Ensure to turn off the listener
        };
    ...
    const sendNotificationRecord = (level, percentage) => {
        ...
        const newNotificationRef = push(notificationRef); // Use push to create a new record
        
        // Get current Nepal local time
        const nepalTime = new Date().toLocaleString("en-US", {
            timeZone: "Asia/Kathmandu" // Adjust timezone to Nepal (Asia/Kathmandu)
        });

        set(newNotificationRef, {
            level: level,
            percentage: percentage,
            timestamp: nepalTime // Use Nepal local time as the timestamp
        })
        .then(() => {
            console.log("Notification record sent to Firebase");
        })
        .catch((error) => {
            console.error("Failed to send notification record: ", error);
        });
    };
